...
export const USER_DATA_DIR = path.join(__dirname, "..", ".auth", "user-data");
export const SCREENSHOT_DIR = path.join(__dirname, "..", ".auth");
export const BASE_URL = "https://bugs.immunefi.com";

// Magnus platform paths (Ref Finance specific — adjust IDs for other orgs/projects)
export const REPORTS_BASE = "/magnus/935/projects/642/bug-bounty/reports";
export const PROGRAM_ID = "435";
...
export function getCredentials(): { email: string; password: string } {
  ...
  if (!email || !password) {
    throw new Error("Missing IMMUNEFI_EMAIL or IMMUNEFI_PASSWORD in .env file.");
  }
...
export async function checkLoginStatus(context: BrowserContext): Promise<boolean> {
...
export async function getReports(context: BrowserContext, filter: string): Promise<ReportItem[]> {
  const page = await context.newPage();
...
export async function getReportDetail(context: BrowserContext, reportUrl: string): Promise<string> {
  const page = await context.newPage();
...
/** Build report detail URL from a numeric ID */
export function buildReportUrl(reportId: string): string {
  const id = reportId.replace(/^#/, "");
  return `${BASE_URL}${REPORTS_BASE}/${id}?programId=${PROGRAM_ID}`;
}

/** Create a persistent browser context and ensure login */
export async function initContext(): Promise<BrowserContext> {
  ensureDirs();
  ...
  if (!loggedIn) {
    ...
    if (!loggedIn) {
      await context.close();
      throw new Error("Failed to log in to Immunefi.");
    }
  }
